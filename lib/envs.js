...
// Match three types of key value pairs in env files, always both single line and multiline
// newline delimited key value (key=value\n) |
// double quote multi-line (key="value") |
// single quote multi-line (key='value')
var envRegularExpression =/([\w]+)=([^"'][^\n]+)[\n]|([\w]+)=["]([^"]+)["]|([\w]+)=[']([^']+)[']/g;

function regKeyValue(data) {
  var env = {};
  var result = envRegularExpression.exec(data.toString());

  while (result) {
    var tripple = result.filter(function(r){return(r);});
    var k = tripple[1];
    var v = tripple[2];
    env[k]=v;
    result = envRegularExpression.exec(data.toString());
  }
  return(env);
}
